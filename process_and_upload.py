...
MIN_QTY = 10
PREFER_0373_DIFF = 20.0  # fino a quanto € di differenza rispetto al prezzo più basso
...
def main():
    ...
    # -----------------------------
    # Applichiamo la scelta finale per ogni EAN
    # -----------------------------
    final_rows = []
    for ean, items in ean_map.items():
        # Prezzo minimo tra tutti i fornitori
        min_item = min(items, key=lambda x: x["prezzo_iva_esclusa"])

        # Verifica preferenza 0373
        prefer_item = None
        for it in items:
            if supplier_from_sku(it["sku"]) == "0373":
                if it["prezzo_iva_esclusa"] <= min_item["prezzo_iva_esclusa"] + PREFER_0373_DIFF:
                    prefer_item = it
                    break

        final_rows.append(prefer_item if prefer_item else min_item)

    if not final_rows:
        raise Exception("❌ Feed vuoto dopo i filtri: upload bloccato!")

    print(f"✅ Prodotti filtrati: {len(final_rows)}")
